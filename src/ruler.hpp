...
class uint8_ruler : public ruler<uint8_t>
{
    ...
    void set_distance(int n0, std::bitset<8> n1, size_t v)
    {
        set_distance(n0, n1.to_ulong(), v);
    }
    void set_distance(std::bitset<8> n0, int n1, size_t v)
    {
        set_distance(n1, n0, v);
    }
    void set_distance(std::bitset<8> n0, std::bitset<8> n1, size_t v)
    {
        set_distance(n0.to_ulong(), n1.to_ulong(), v);
    }

    bool known(int x, int y)
    {
        return known_[x][y];
    }
  ...
  public:
    ...
    virtual size_t distance(uint8_t x, uint8_t y) const
    {
...
class uint16_ruler : public ruler<uint16_t>
{
  ...
  public:
    ...
    {
        return byte_ruler_.distance(x >> 8, y >> 8) + byte_ruler_.distance(x & 0xff, y & 0xff);
    }
...
class uint32_ruler : public ruler<uint32_t>
{
  ...
  public:
    ...
    {
        return byte_ruler_.distance(x >> 24, y >> 24) + byte_ruler_.distance((x >> 16) & 0xff, (y >> 16) & 0xff) +
               byte_ruler_.distance((x >> 8) & 0xff, (y >> 8) & 0xff) + byte_ruler_.distance(x & 0xff, y & 0xff);
    }

    static const uint32_ruler ruler;
};

template <typename T> class bit_ruler : public ruler<T>
{
