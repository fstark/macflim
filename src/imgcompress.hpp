...
template <typename T> void write2(T &out, uint32_t v)
{
    write1(out, v / 256);
    write1(out, v % 256);
}

template <typename T> void write4(T &out, uint32_t v)
{
    write2(out, v / 65536);
    write2(out, v % 65536);
}
...
template <typename T> void write(T &out, const std::vector<uint32_t> &vec)
{
    for (auto v : vec)
        write4(out, v);
}

template <typename T> void write(T &out, const std::vector<uint8_t> &vec)
{
    for (auto v : vec)
        write1(out, v);
}

template <typename T, size_t N> void write(T &out, const std::array<uint8_t, N> &arr)
{
    for (auto v : arr)
        write1(out, v);
}
...
template <typename T>
inline std::vector<run<T>>
pack(typename std::vector<T>::const_iterator data,
     std::vector<bool>::const_iterator pack_begin, //  Given in real screen order, probably a mistake
     std::vector<bool>::const_iterator
         pack_end, //  (offset_t does the automatic conversion, so we scan in vertical order)
     size_t max_pack_bytes, size_t width, size_t height)
{
