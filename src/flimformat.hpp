#ifndef FLIMFORMAT_INCLUDED__
#define FLIMFORMAT_INCLUDED__
...
struct flim_info
{
    ...
    flim_info( const encoding_profile &profile, const std::vector<flimcompressor::frame> frames )
    {
        width_ = profile.width();
        height_ = profile.height();
        silent_ = profile.silent();
        frame_count_ = frames.size();
        total_ticks_ = std::accumulate( std::begin(frames), std::end(frames), 0, []( size_t a, const flimcompressor::frame &f ){ return a+f.ticks; } );
        byterate_ = profile.byterate();
    }
...
public:
    ...
    void add_framebuffer( eComponentType type, const framebuffer &fb )
    {
        ...
        write2( bi, fb.W() ); 
        write2( bi, fb.H() ); 
        auto img_data = fb.raw_values_natural<u_int8_t>();
    ...
    {
        std::vector<u_int8_t> data = fb.raw_values_natural<u_int8_t>();
