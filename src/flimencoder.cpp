...
std::vector<uint8_t> flimencoder::normalize_sound(std::vector<double> sound_samples, size_t len)
{
    ...
    if (sound_samples.size() > 0)
    {
        ...
        std::transform(std::begin(sound_samples), std::end(sound_samples), std::back_inserter(res),
                       [&](double v) { return clamp((v / scale) * 128 + 128, 0, 255); });
...
void flimencoder::set_fps(double fps)
{
    fps_ = fps;
}
...
void flimencoder::set_poster_ts(double poster_ts)
{
    poster_ts_ = poster_ts;
}
void flimencoder::set_subtitles(const std::vector<subtitle> &subtitles)
{
    subtitles_ = subtitles; /* yes, it is a copy */
}

//  Encode all the frames
void flimencoder::make_flim(const std::string flim_pathname, input_reader *reader,
                            ...
                            const std::vector<std::unique_ptr<output_writer>> &writers)
{
    ...
    flim_info fi{profile_.width(), profile_.height(), profile_.silent(),
